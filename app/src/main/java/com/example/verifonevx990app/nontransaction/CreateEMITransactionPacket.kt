...
class CreateEMITransactionPacket(private var cardProcessedData: CardProcessedDataModal, var emiCustomerDetails: EmiCustomerDetails?) : ITransactionPacketExchange {
    ...
    override fun createTransactionPacket(): IsoDataWriter = IsoDataWriter().apply {
        ...
        if(terminalData != null) {
            ...
            mti = if (!TextUtils.isEmpty(AppPreference.getString(AppPreference.GENERIC_REVERSAL_KEY))) {
                Mti.REVERSAL.mti
            } else {
        ...
        //    if(null !=cardProcessedData.getPosEntryMode().toString() && cardProcessedData.getPosEntryMode().toString().isNotEmpty())
            ...
            addField(23 , addPad(cardProcessedData.getApplicationPanSequenceValue().toString() , "0" , 6 , true))
